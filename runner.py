...
parser.add_argument('--graph_type', dest='graph_type', help='Gets the type of graph')
parser.add_argument('--nodes_num', dest='nodes_num', help='Gets the type of graph')
parser.add_argument('--k_regular', dest='k_regular', help='Gets the type of graph')
...
parser.add_boolean('--network-loops', dest='network_loops', default=False,
...
def generate_graph() -> [nx.Graph, nx.DiGraph]:
    ...
    if args.nodes_num is not None:
        args.nodes_num = int(args.nodes_num)

    if args.graph_type == "complete":
        g = nx.complete_graph(range(1, 1+args.nodes_num))
    elif args.graph_type == "cycle":
        g = nx.cycle_graph(range(1, 1+args.nodes_num))
    elif args.graph_type == "path":
        g = nx.path_graph(range(1, 1+args.nodes_num))
    elif args.graph_type == "star":
        g = nx.star_graph(range(args.nodes_num, 0, -1))
    elif args.graph_type == "k_regular":
        g = nx.random_regular_graph(int(args.k_regular), args.nodes_num)
        g = nx.relabel_nodes(g, {0: 6})
    else:
