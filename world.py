...
class AbstractWorld(object):
    ...
    def send_wave_to_neighbours(self, parent, wave):
        raise NotImplementedError

    def send_wave_to_parent(self, parent, wave):
        raise NotImplementedError
...
class SimulatorFullView(AbstractWorld):
    ...
    def receive(self, src, msg):
        from algorithm import  process_wave_msg
        return process_wave_msg(src, msg)
    ...
    def send_wave_to_neighbours(self, parent, wave):
        log(f'send wave to neighbours. parent:{parent}    wave:{wave}')
        counter = 0
        for node in self.neighbors:
            if node != parent:
                counter += self.send_message(node, wave)
        return counter

    def send_wave_to_parent(self, parent, wave):
        log(f'send wave to parent. parent:{parent}    wave:{wave}')
        return self.send_message(parent, wave)
...
class SimulatorOnlyNeighbors(SimulatorFullView):
    ...
    def send_wave_to_neighbours(self, parent, wave):
        log(f'send wave message. parent:{parent}    wave:{wave}')
        counter = 0
        for node in self.neighbors:
            if node != parent:
                counter += self.send_message(node, wave)
